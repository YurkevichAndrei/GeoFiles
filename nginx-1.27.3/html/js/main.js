...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let wmsSource1 = new ol.source.TileWMS({
        ...
        params: {'LAYERS': [`geoapp:test__kml`], 'VERSION':'1.3.0', 'TILED': true, 'TRANSPARENT': true},
    ...
    document.getElementById("search-input").addEventListener("keyup", function(event) {
        if (event.key === "Enter") {
            console.log("Поиск: ", event.target.value); // Здесь можно добавить логику поиска
        }
    });
        ...
        fetch('/server/files')
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    ...
                    title.textContent = item || 'Без названия';
                    ...
                    for (let i=0; i<layers.getLength(); i++)
                    {
                        ...
                        if (typeof layerParams !== "undefined")
                        {
                            ...
                            if (item === layerName)//слой на карте присутствует
                            {
                                existLayer = true;
                                break;
                            }
                    ...
                    downloadButton.onclick = () => downloadItem(item); // Функция для скачивания элемента
    ...
    //         url: 'http://127.0.0.1:5151/geoserver/geoapp_test/wms',
    //         params: {'LAYERS': [`geoapp_test:${name}`], 'TILED': true},
    //         serverType: 'geoserver'
    //     });
